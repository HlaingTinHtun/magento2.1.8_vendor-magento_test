...
class Attributes extends \Magento\TargetRule\Model\Rule\Condition\Product\Attributes
{
    ...
    public function getConditionForCollection($collection, $object, &$bind)
    {
        ...
        if ($attribute->isStatic()) {
            ...
            /*
             * The 'catalog_product_relation' table added to allow select parent product entities
             * by child products.
             *
            ...
            $select = $object->select()
                ->from(
                    ...
                    $resource->getConnection()->getIfNullSql(
                        'relation.parent_id',
                        'table.' . $linkField
                    )
                )
                ->joinLeft(
                    ['relation' => $resource->getTable('catalog_product_relation')],
