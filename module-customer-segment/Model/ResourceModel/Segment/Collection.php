...
class Collection extends \Magento\Rule\Model\ResourceModel\Rule\Collection\AbstractCollection
{
     ...
     *
     * @param int|string|array|null $customerCount
     * @return $this
     */
    public function addCustomerCountFilter($customerCount)
    {
        $this->addCustomerCountToSelect();
        $this->addCustomerCountFilterToSelect($customerCount);
    ...
    /**
     * Add customer_count filter to collection.
     *
     * @param int|string|array|null $value
     * @return void
     */
    private function addCustomerCountFilterToSelect($value)
    {
        if (is_array($value)) {
            if (isset($value['from']) && isset($value['to'])) {
                $this->getSelect()->having(
                    sprintf(
                        'customer_count BETWEEN %d AND %d',
                        (int)$value['from'],
                        (int)$value['to']
                    )
                );
            } elseif (isset($value['from'])) {
                $this->getSelect()->having('customer_count >= ?', (int)$value['from']);
            } elseif (isset($value['to'])) {
                $this->getSelect()->having('customer_count <= ?', (int)$value['to']);
            }
        } else {
            $this->getSelect()->having('customer_count = ?', (int)$value);
        }
    }
